...
public class GamePanel extends JPanel implements Runnable, KeyListener {
		...
		for(int i = 0; i < enemies.size(); i++)
		{
			...
			{	
				...
				if(rand < 0.001) 
					...
					(rand < 0.020) powerups.add(new PowerUp(3 , e.getx(), e.gety()));
				else if 
					(rand < 0.120) powerups.add(new PowerUp(2 , e.getx(), e.gety()));
				else if
					(rand < 0.130) powerups.add(new PowerUp(4, e.getx(), e.gety()));
	...
	private void gameRender(){
		...
		if(waveStartTimer != 0)
		{
			g.setFont(new Font("Centry Gothic", Font.PLAIN, 25));
		...
		{
			g.setColor(Color.WHITE);
			g.fillOval(20 + (40 * i), 20, player.getr() * 2, player.getr() * 2);
			g.setStroke(new BasicStroke(3));
			g.setColor(Color.WHITE.darker());
			g.drawOval(20 + (40 * i), 20, player.getr() * 2, player.getr() * 2);
			g.setStroke(new BasicStroke(1));
		}

		//draw player power
		g.setColor(Color.YELLOW);
		g.fillRect(20, 80 , player.getPower() * 16, 16);
		g.setColor(Color.YELLOW.darker());
		g.setStroke(new BasicStroke(2));
		for(int i = 0; i < player.getRequiredPower(); i++){
			g.drawRect(20 + 16 * i, 80, 16 , 16);
		}
		g.setStroke(new BasicStroke(1));
		...
		if(slowDownTimer != 0) {
			g.setColor(Color.WHITE);
			g.drawRect(20, 60, 100, 16);
			g.fillRect(20, 60,
				(int)(100 - 100.0 * slowDownTimerDiff / slowDownLength), 16);
		}
	}
